...
import * as Modals from './modalManager.js';
  ...
  const _docOpenKey = (sessionId) => 'penelope-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'penelope-doc-minimized-' + sessionId;
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('penelope-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('penelope-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'penelope-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('penelope-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('penelope-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('penelope-suggestions-' + docId);
      if (!raw) return;
