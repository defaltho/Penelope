  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Penelope.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('Penelope app', 'Run inside the Penelope app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Penelope keys, choose model directories, and set the default server. Local is this machine.</p>';
