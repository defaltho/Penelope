# Penelope Claude Code Integration

This directory contains the Claude Code skill bundle for Penelope.

## User Flow

1. Open Penelope Settings > Integrations.
...
available in any session that has `PENELOPE_URL` and `PENELOPE_API_TOKEN` in its
environment.

## What's in the bundle

- `skills/penelope/SKILL.md` — the skill definition Claude Code reads.
- `skills/penelope/scripts/penelope_api.py` — small helper that calls the scoped
...
The token is scope-gated. Every tool surface is checked server-side in Penelope,
